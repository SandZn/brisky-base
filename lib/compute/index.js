...
exports.compute = function (val, previousValue, stamp, origin) {
  if (val === void 0) {
    val = this.val
    // can save some speed here do we want compute else it works really well for speed
  }
  if (val !== void 0) {
    let type = typeof val
    if (type === 'function') {
      val = val.call(this.getBind(previousValue, stamp), previousValue)
    } else if (type === 'object' && val._base_version) {
...
exports.origin = function () {
  ...
  while (typeof val === 'object' && val._base_version) {
    reference = val
    val = reference.val
  }
  return reference
