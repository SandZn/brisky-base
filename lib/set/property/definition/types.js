...
exports.createPropertyConstructor = function (Constructor, key, override) {
  ...
  function propertyConstructor (val, stamp, resolve, nocontext, type) {
    ...
    if (val && val.noReference && (!val._parent || val._parent === this)) {
      // experimental -- may create strange results
      val._key = key
      val._parent = this
...
exports.custom = function (properties, property, key, stamp) {
  if (property.val || property.override) {
    ...
    if (prototype && typeof prototype === 'object' && prototype.isBase) {
      ...
      if (!property.override && Object.keys(property).length > 1) {
        defaultConstructor(this, property, stamp, key, properties)
      } else {
        if (property.override) {
...
function defaultConstructor (target, property, stamp, key, properties) {
  if (property.type) {
