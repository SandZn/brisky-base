...
function defineProperty (target, key, val, props, stamp) {
  ...
  if (mapKey) {
    move(target, key, props, stamp, prop, mapKey)
    addToKeyMap(props, key, mapKey)
  }
  if (hasVal) {
    props[key] = property(target, targetKey, val.val, props, stamp, prop, key) || null
  } else if (prop) {
    if (mapKey || keyIsNull) {
      if (prop.base) {
        prop = props[key] = baseProperty(prop.base, targetKey)
      } else {
        prop = props[key] = mapDefaultProperty(targetKey)
      }
    }
    if (keyIsNull) {
