...
exports.add = function addFilter (target, key, copy, sort, type) {
  var filters = target._filters
  const start = !type
  if (copy) {
    target._filters = filters = copyFilters(filters) // not covered in tests
  }
  if (start) {
  ...
  if (start && keysFilter(target, key)) {
    addFilter(target, key, copy, sort, 'keys')
  }
}
