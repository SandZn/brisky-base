'use strict'
module.exports = function (base) {
  // @todo add this in the functions themselves this costs A LOT of perf!
  var _addProperty = base.addNewProperty
  var _remove = base.remove
  var _removeUpdateParent = base.removeUpdateParent
  var _removeProperty = base.removeProperty

  exports.properties = {
    _keylists: { val: [ '_keys' ] },
    order (val, stamp) {
      if (!this.order) {
        return this.setKeyInternal('order', val, stamp)
      } else {
        let change = this.setKeyInternal('order', val, stamp)
        if (change && this._parent) {
          this._parent.clearKeysCache()
        }
        return change
      }
    }
  }

  exports.define = {
    keysCheck (val, key) {
      var target = val[key]
      return target &&
        target._base_version &&
        key !== 'listensOnBase' &&
        key !== 'listensOnAttach'
    },
    removeUpdateParent () {
      if (this._parent) {
        this._parent.clearKeysCache()
      }
      return _removeUpdateParent.apply(this, arguments)
    },
    keys (field, check) {
      if (!check) {
        check = this.keysCheck
      }
      if (!field) {
        field = '_keys'
      }
      var keys = this[field]
      if (!keys && keys !== false || !this.hasOwnProperty(field)) {
        keys = this[field] = []
        let ordered
        let target
        for (let key in this) {
          target = this[key]
          if (key[0] !== '_' && ((target = this[key]), target !== null) && check(this, key)) {
            if (!ordered && target.order) {
              ordered = true
            }
            keys.push(key)
          }
        }
        if (ordered) {
          // .sort is very slow fix this using a custom qsort
          keys.sort(this.sort.bind(this))
        } else if (!keys[0]) {
          keys = this[field] = false
        }
      }
      return keys
    },
    sort (a, b) {
      a = this[a].order || 0
      b = this[b].order || 0
      a = a.compute && a.compute() || a || 0
      b = b.compute && b.compute() || b || 0
      return a === b ? 0 : a < b ? -1 : 1
    },
    remove () {
      if (this.parent) {
        this.parent.clearKeysCache()
      }
      return _remove.apply(this, arguments)
    },
    removeProperty () {
      this.clearKeysCache()
      return _removeProperty.apply(this, arguments)
    },
    addNewProperty () {
      var ret = _addProperty.apply(this, arguments)
      this.clearKeysCache()
      return ret
    },
    clearKeysCache () {
      for (let i = 0, len = this._keylists.length; i < len; i++) {
        let keylist = this._keylists[i]
        if (this[keylist] !== void 0) {
          this[keylist] = null
        }
      }
    }
  }
  base.set(exports)
}
