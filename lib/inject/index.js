...
exports.inject = function () {
  ...
  var stamp
  var stamptarget = arguments[length - 1]
  var type = !stamptarget || typeof stamptarget
  // console.log('here we go inject!', stamp, type, stamptarget)
  if (
...
function inject (val, stamp) {
  ...
  if (isFn) {
    ...
    if (this instanceof Base || this === Base.prototype) {
      // make your own stamp something like inject -- then correct it for new?
      this.set(val, stamp)
    } else {
      if (this.define) {
        this.define(val)
