...
module.exports = function (target, val, stamp, excludes) {
  ...
  for (let i in properties) {
    if (properties[i] !== myProperties[i] && i !== 'keyMap') {
      if (!newProps) { newProps = {} }
      newProps[i] = { val: properties[i].base || properties[i] }
    }
  }
  if (newProps) {
    if (keyMap) {
